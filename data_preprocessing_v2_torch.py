...
def prepare_train_data():
    ...
    try:
        ...
        x_set = h5f.create_dataset("x", shape=(target_num, 1, window),
                                   maxshape=(None, 1, window), chunks=(32, 1, window))
        spec_set = h5f.create_dataset("spectrogram", shape=(target_num, 1, nfft // 2, nfft // 2),
                                      maxshape=(None, 1, nfft // 2, nfft // 2),
                                      chunks=(32, 1, nfft // 2, nfft // 2))
        label_set1d = h5f.create_dataset("label1d", shape=(target_num, 1), maxshape=(None, 1), chunks=(32, 1))
        ...
        x_set.resize([target_num + i0, 1, window])
        spec_set.resize([target_num + i0, 1, nfft // 2, nfft // 2])
        label_set1d.resize([target_num + i0, 1])
...
def prepare_validation_data(func):
    ...
    return x_val, spec_val, labels[:, np.newaxis]
...
if __name__ == '__main__':
    ...
    if False:
        ...
        x_train_for_val, spec_train_for_val, y_train_for_val = prepare_validation_data(read_one_folder)
        ...
        with open('val_torch.pkl', 'wb') as f:
            pickle.dump(d, f)
